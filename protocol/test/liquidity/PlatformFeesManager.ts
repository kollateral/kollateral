...
describe('PlatformFeesManager', () => {
	...
	beforeEach(async () => {
		...
		PlatformFeesManagerAggregator =
			await PlatformFeesManagerAggregatorFactory
				.connect(owner).deploy(100, FEE_COLLECTION_ADDRESS);

		await PlatformFeesManagerAggregator.deployed();
	});

	it('getPlatformFeeBips should return 100', async () => {
		expect(await PlatformFeesManagerAggregator.connect(user)
			.getPlatformFeeBips()).to.be.equal(100);
	});

	it('getPlatformFeeCollectionAddress should return correct address', async () => {
		expect(await PlatformFeesManagerAggregator.connect(user)
			.getPlatformFeeCollectionAddress()).to.be.equal(FEE_COLLECTION_ADDRESS);
	});

	it('calculatePlatformFee should return correct fee applied to amount param', async () => {
		expect(await PlatformFeesManagerAggregator.connect(user)
			.calculatePlatformFee(10000)).to.be.equal(100);
	});

	it('setPlatformFeeBips cannot be called by non owner address', () => {
		expect(PlatformFeesManagerAggregator.connect(user)
			.setPlatformFeeBips(100))
			.to.be.revertedWith('Ownable: caller is not the owner');
	});

	it('setPlatformFeeCollectionAddress cannot be called by non owner address', () => {
		expect(PlatformFeesManagerAggregator.connect(user)
			.setPlatformFeeCollectionAddress(FEE_COLLECTION_ADDRESS_NEW))
			.to.be.revertedWith('Ownable: caller is not the owner');
	});
	...
	it('setPlatformFeeCollectionAddress succeeds when called by owner', async () => {
		...
		expect(await PlatformFeesManagerAggregator.connect(user)
			.getPlatformFeeCollectionAddress()).to.be.equal(FEE_COLLECTION_ADDRESS_NEW);
	});
