...
describe('SoloLiquidityProxy', () => {
    ...
    describe('for Ether borrows', () => {
        ...
        beforeEach(async () => {
            ...
            await Invoker.connect(owner).setLiquidityProxies(ETHER_TOKEN_ADDRESS, [SoloLiquidityProxy.address, KingmakerLiquidityProxy.address]);
            ...
            let testInvokableTx = await owner.sendTransaction({
        ...
        describe('invoke', () => {
            ...
            describe('when calling estimateRepaymentAmount', () => {
                it('returns correct amount for single pool', async () => {
                    expect(await Invoker.estimateRepaymentAmount(ETHER_TOKEN_ADDRESS, kEtherUnderlyingBalance))
                        .to.be.equal(ethers.BigNumber.from(100051));
                });

                it('returns correct amount for multi pool', async () => {
                    expect(await Invoker.estimateRepaymentAmount(ETHER_TOKEN_ADDRESS, kEtherUnderlyingBalance.mul(2)))
                        .to.be.equal(ethers.BigNumber.from(200351));
                });

                it('reverts for too high amount', async () => {
                    await expect(Invoker.estimateRepaymentAmount(ETHER_TOKEN_ADDRESS, kEtherUnderlyingBalance.mul(3)))
                        .to.be.revertedWith("Invoker: not enough liquidity");
                });

                it('reverts for non-registered tokens', async () => {
                    await expect(Invoker.estimateRepaymentAmount(OTHER_TOKEN_ADDRESS, ethers.BigNumber.from(0)))
                        .to.be.revertedWith("Invoker: no liquidity for token");
                });
            });
        });
    });
